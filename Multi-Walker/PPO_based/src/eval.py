...
# set font path
font_path = "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc"  # ensure the path is correct
...
def eval(
    ...
    def run_episode(episode_idx: int):
        ...
        for agent in env.agent_iter():
            ...
            env.step(act)
        env.reset()
        episode_data["steps"] = step // n_walkers
        env.close()
    ...
    # Use joblib to execute episodes in parallel
    # verbose = 100
    ...
    for result in results:
        ...
        if save_gif and len(frames) > 0:
            ...
            print(f"save gif {timestamp_str} {result['rewards']}")
...
def run_evaluations(cfg: DictConfig, checkpoint_path: dict) -> tuple[dict, dict]:
    """Run baseline and disturbance evaluation"""
    ...
    # Run disturbance test evaluation
...
def load_eval_results(filepath: str) -> dict:
    """Load evaluation results from JSON file and save a copy to hydra output directory"""
    with open(filepath, "r") as f:
        data = json.load(f)

    # Save a copy to hydra output directory
...
def analyze_eval_results(
    ...
    """Analyze evaluation results and generate report and charts"""
    console = Console()
    ...
    # Count the number of angle intervals
    angle_intervals = [
        (0, 1),
        (1, 2),
        (2, 3),
    ...
    # Generate report
    output = f"""
    [bold cyan]Evaluation results - {checkpoint_path.desc}[/]
    
    [bold]Average reward:[/] {format_metric_comparison("reward", baseline_results["reward"], disturb_results["reward"], 2)}
    [bold]Average angle fluctuation:[/] {format_metric_comparison("angle", baseline_results["angle"], disturb_results["angle"], 5)}
    [bold]....angle fluctuation:[/] {format_metric_comparison("angle_deg", baseline_results["angle_deg"], disturb_results["angle_deg"], 5)}
    [bold]Average steps:[/] {format_metric_comparison("steps", baseline_results["steps"], disturb_results["steps"], 1)}
    
    [bold]Failure fall count:[/] {format_metric_comparison("terminate_cnt", baseline_results["terminate_cnt"], disturb_results["terminate_cnt"], 1)}

    [bold]Angle interval statistics:[/]
    [bold]0-1 degree count:[/] {format_metric_comparison("angle_0_1", baseline_interval_counts["0-1"], disturb_interval_counts["0-1"], 1)}
    [bold]1-2 degree count:[/] {format_metric_comparison("angle_1_2", baseline_interval_counts["1-2"], disturb_interval_counts["1-2"], 1)}
    [bold]2-3 degree count:[/] {format_metric_comparison("angle_2_3", baseline_interval_counts["2-3"], disturb_interval_counts["2-3"], 1)}
    [bold]3-5 degree count:[/] {format_metric_comparison("angle_3_5", baseline_interval_counts["3-5"], disturb_interval_counts["3-5"], 1)}
    [bold]5-8 degree count:[/] {format_metric_comparison("angle_5_8", baseline_interval_counts["5-8"], disturb_interval_counts["5-8"], 1)}
    [bold]8-10 degree count:[/] {format_metric_comparison("angle_8_10", baseline_interval_counts["8-10"], disturb_interval_counts["8-10"], 1)}
    [bold]10-15 degree count:[/] {format_metric_comparison("angle_10_15", baseline_interval_counts["10-15"], disturb_interval_counts["10-15"], 1)}
    [bold]>15 degree count:[/] {format_metric_comparison("angle_15_inf", baseline_interval_counts["15-inf"], disturb_interval_counts["15-inf"], 1)}
    
    [bold]Same angle count:[/] {same_count // 3}
    [bold]Evaluation count:[/] {cfg.eval_episodes}
    """

    panel = Panel(
        output.strip(),
        title="[bold]Evaluation report",
        title_align="center",
        padding=(1, 4),
        style="blue",
    )
    ...
    # Draw angle distribution comparison chart
    ...
    ax_base.set_title("Baseline angle distribution", fontproperties=font_prop)
    ax_base.set_ylabel("cnt")
    ax_base.tick_params(axis="x", rotation=45)

    # Draw disturb distribution
    ax_disturb.bar(labels, disturb_values, color="red", alpha=0.6)
    ax_disturb.set_title("Disturb angle distribution", fontproperties=font_prop)
    ...
    # Now rewards_curve is (steps, n_walkers, 3)
    # Need to merge it into (steps, 3)
    ...
    # Draw another cumulative reward curve
...
def main(cfg: DictConfig):
    ...
    def process_checkpoint(checkpoint_path):
        ...
        if cfg.get("load_results", False):  # Load existing results mode
            ...
            # Save results to timestamp subdirectory
            save_eval_results(
                baseline_results,
                timestamp_dir,
            ...
            # Also save the latest version to the latest subdirectory
            save_eval_results(
        ...
        # Analyze results and get charts
    ...
    with Progress() as progress:
        ...
        for checkpoint_path in cfg.checkpoint_path:
            ...
            end_time = time.time()
            print(
                f"Processing checkpoint {checkpoint_path.desc} took: {end_time - start_time:.2f} seconds"
            )

    def _draw():
        # Create a new chart to compare all checkpoints
        compare_fig, ((cax1, cax2), (cax3, cax4)) = plt.subplots(2, 2, figsize=(15, 10))

        # Set the title and range of the comparison chart
